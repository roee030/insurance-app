        ...
        {signed ? (
          ...
          <>
            <div className="py-5">
              ...
              <p className="mt-1 text-[17px] leading-relaxed text-slate-600">
            ...
            {view.productActions.length > 0 && (
              <div className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="mb-2 text-[16px] font-semibold text-slate-700">
                  פרטי הבקשה
                </div>
                <div className="space-y-2">
                  {view.productActions.map((a) => (
                    <div
                      key={a.id}
                      className="rounded-lg border border-slate-200 bg-white p-3 text-[17px]"
                    >
                      <div className="mb-1.5 font-medium text-slate-800">
                        {a.productType}
                        <span className="mr-1 text-[15px] font-normal text-slate-500">
            ...
            <label className="block text-[16px] font-medium text-slate-600">
              שם מלא לחתימה
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="הקלד/י את שמך המלא"
                className="mt-1 h-11 w-full rounded-xl border border-slate-300 px-3 text-[19px] outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
              />
            </label>
            ...
            <button
              ...
              className="mt-4 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 text-[19px] font-semibold text-white transition hover:bg-emerald-500 disabled:opacity-40"
            >
              {signing ? (
                <>
